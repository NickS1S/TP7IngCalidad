...
describe('HomePage - Pruebas Unitarias', () => {
  ...
  test('debería renderizar el título principal', () => {
    render(<HomePage />);
    expect(screen.getByText(/Pipeline CI\/CD - Ingeniería de Software/i)).toBeInTheDocument();
  });
  ...
  test('debería mostrar errores de validación cuando faltan campos', () => {
    render(<HomePage />);
    const botonAgregar = screen.getByRole('button', { name: /agregar usuario/i });
  ...
  test('debería mostrar estadísticas cuando hay usuarios', () => {
    ...
    fireEvent.change(inputNombre, { target: { value: 'Usuario Estadísticas' } });
  ...
  test('debería renderizar el footer del proyecto', () => {
    render(<HomePage />);
    expect(screen.getByText(/universidad nacional de cuyo/i)).toBeInTheDocument();
